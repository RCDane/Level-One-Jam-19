...
public class VoxelHair : MonoBehaviour
{
    ...
    void CreateVoxelPool()
    {
        ...
        {
            tempObj = Instantiate(testObject, this.transform.parent);
    ...
    public void RemoveHair(Vector3 position, float distance)
    {
        ...
        for (int i = 0; i < VoxelsToGoThrough.Count; i++)
        {
            ...
            //{
            //RemoveVoxel();
            RemoveVoxel(VoxelsToGoThrough[i].pos);
    ...
    void DeactivateVoxel(GameObject obj)
    {
        if (obj != null)
        {
            VoxelObj temp = obj.GetComponent<VoxelObj>();
            activeVoxels.Remove(temp);
            temp.Deactivate();
            inActiveVoxels.Push(temp);
        }
